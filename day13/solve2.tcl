...
proc step {} {
	...
	dict for {x lineData} $map {
		dict for {y cellData} $lineData {
			# access global $map, loop might have changed it
			if {![dict exists $cellData cart] || ![dict exists $map $x $y cart]} {
				continue
			}
			...
			puts "Moving ($x,$y) $direction to ([join $dst ,]) [dict get $map {*}$dst track]"
			if {[dict exists $map {*}$dst cart]} {
				puts "Crash on: $dst"
				dict unset map {*}$dst cart
...
#	}
#	after 500
	puts "Step [incr step] ($carts carts)"
	step
}
dict for {x lineData} $map {
	dict for {y cellData} $lineData {
		if {[dict exists $cellData cart]} {
			puts "And the winner is: [dict get $cellData cart direction] @ ($x,$y)"
			exit 0
		}
	}
}
